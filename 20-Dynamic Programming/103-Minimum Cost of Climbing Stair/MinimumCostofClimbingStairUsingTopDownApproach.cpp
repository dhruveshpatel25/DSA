...
int solve(int level,vector<int>& cost,vector<int>& dp){
    ...
    //return for other level if there is a value
    if(dp[level]!=-1){
        return dp[level];
    }

    //cost of that level + minimum from below level
    dp[level]=cost[level]+min(solve(level-1,cost,dp),solve(level-2,cost,dp));
    return dp[level];
}
